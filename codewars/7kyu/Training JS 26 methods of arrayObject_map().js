...
// The original array should not be changed, you need to return a 
// new array(if you use the map method, you do not need to worry about this).
// Example
...
function isolateIt(arr){
  ...
  return arr.slice()
          .map(item => item.slice(0, Math.floor(item.length / 2)) + "|" + item.slice(Math.ceil(item.length / 2)));
}
